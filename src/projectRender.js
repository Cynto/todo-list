...
const addProject = (() => {
  ...
  addProjectButton.setAttribute(
    'style',
    'font-size: 50px; justify-content: center; font-weight: bolder;',
  );
  ...
  addProjectButton.addEventListener('click', () => {
    ...
    confirmButton.addEventListener('click', () => {
      ...
      const projectTitles = projectsArray.map((title) => title.title);
...
const projectRenderer = (() => {
  ...
  const renderPage = () => {
    ...
    for (let i = 0; i < itemArray.array.length; i += 1) {
      if (itemArray.array[i].complete === false) {
        ...
        circleIcon.addEventListener('click', () => {
          ...
          const found = projectsArray.find(
            (item) => item.dataProject === itemArray.dataProject,
          );
          if (found !== undefined) {
        ...
        dueDate.addEventListener('click', () => {
          ...
          dueDateInput.addEventListener('change', () => {
            itemArray.array[i].dueDate = format(
